...
{
    [self showImageNoAutoDismiss:WSProgressHUDSuccessDefaultImage() status:string maskType:WSProgressHUDMaskTypeClear maskWithout:WSProgressHUDMaskWithoutNavigation];
}

+ (void)showErrorWithStatus: (NSString *)string
{
    //    [self showImage:WSProgressHUDErrorDefaultImage() status:string];
    [self showImageNoAutoDismiss:WSProgressHUDErrorDefaultImage() status:string maskType:WSProgressHUDMaskTypeClear maskWithout:WSProgressHUDMaskWithoutNavigation];
}
...
/**
 自己加的一个关掉自动消失的需求
 */
+ (void)showImageNoAutoDismiss:(UIImage *)image status:(NSString *)title maskType: (WSProgressHUDMaskType)maskType maskWithout: (WSProgressHUDMaskWithoutType)withoutType {
    [[self shareInstance] addOverlayViewToWindow];
    [[self shareInstance] showImageNoAutoDismiss:image status:title maskType:maskType maskWithout:withoutType];
}
    ...
    [UIView animateWithDuration:WSProgressHUDDismissDuration
                     ...
                     completion:^(BOOL finished){
                         ...
                         if (self.hudView.alpha == 0) {
                             ...
                             // 干掉点击手势
                             if (self.overlayView.gestureRecognizers.count > 0) {
                                 [self.overlayView removeGestureRecognizer:self.overlayView.gestureRecognizers[0]];
                             }
                             
                             if (self.gestureRecognizers.count > 0) {
                                 [self.overlayView removeGestureRecognizer:self.gestureRecognizers[0]];
                             }
...
CG_INLINE UIImage * WSProgressHUDSuccessDefaultImage()
{
    if (WSProgressHUDSuccessImage == nil) {
        NSString *imageName = [NSString stringWithFormat:@"success@%ldx", (long)[UIScreen mainScreen].scale];
        UIImage *successImage = WSProgressHUDImageWithName(imageName, @"png");
...
CG_INLINE UIImage * WSProgressHUDErrorDefaultImage()
{
    if (WSProgressHUDErrorImage == nil) {
        NSString *imageName = [NSString stringWithFormat:@"error@%ldx", (long)[UIScreen mainScreen].scale];
        UIImage *failurImage = WSProgressHUDImageWithName(imageName, @"png");
...
{
    if (!_imageView) {
        _imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 22, 25)];
