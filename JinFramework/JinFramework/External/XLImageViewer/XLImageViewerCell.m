...
//加载动画
@property (strong, nonatomic) XLImageLoading *loading;
...
-(void)buildUI{
    ...
    //添加长按方法
    UILongPressGestureRecognizer *longPress = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(showSaveSheetView:)];
...
/**
 长按方法弹出保存窗口
 */
- (void)showSaveSheetView:(UILongPressGestureRecognizer *)gestureRecognizer {
...
-(void)setImageUrl:(NSString *)imageUrl{
    ...
    @weakify(self)
    //显示网络图片
    [_imageView setImageWithURL:[NSURL URLWithString:imageUrl] placeholder:nil options:YYWebImageOptionUseNSURLCache progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        @strongify(self)
        dispatch_async(dispatch_get_main_queue(), ^{
            [self.loading show];
            self.loading.progress = (CGFloat)receivedSize / (CGFloat)expectedSize;
        });
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    if (error != NULL) {
        [XLImageLoading showAlertInView:self message:@"相册权限未开启"];
        return;
    }
